...
func TestDeriveConfigPopulatesFields(t *testing.T) {
	...
	ctx := models.BuildContext{
		Spec: models.SandboxSpecification{
			...
			DomainProfile: models.DomainProfile{
				Arch:         "amd64",
				Machine:      &machine,
				VCPUs:        4,
				RAMMB:        8192,
	...
	if len(cfg.ExtraArgs) != len(ctx.Spec.DomainProfile.ExtraArgs) {
...
func (noopEnv) Cleanup(models.BuildContext) error { return nil }
